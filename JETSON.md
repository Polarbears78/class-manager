...
**한글·PDF 어느 쪽으로 받으셔도 됩니다** — 콘솔이 알아서 그림을 꺼내 보냅니다.
...
| 걸리는 시간 | 학생 한 명당 몇 분 (한글은 글줄 그림 40~50장, PDF는 쪽수만큼). 한 번만 읽으면 되고 진행 막대로 몇 장째인지 보입니다 |
...
| 응답이 너무 느리다 | 더 작은 모델로 바꾸기 · 고급 설정의 **분석에 넣을 최대 글자 수** 줄이기 · 넣는 영역 줄이기 · `nvpmodel -m 0` |
| 긴 생기부에서 메모리가 부족하다 | 기본값이 **10,000자씩 나눠 읽기**라 한 번에 보내는 양은 크지 않습니다. 그래도 버거우면 **한 번에 읽을 글자 수**를 5,000 정도로 더 줄여 보세요 |
| 나눠 읽으니 너무 오래 걸린다 | 조각 수만큼 서버를 부릅니다. **한 번에 읽을 글자 수**를 키우거나(문맥이 커짐) **분석에 넣을 최대 글자 수**를 줄이세요 |
